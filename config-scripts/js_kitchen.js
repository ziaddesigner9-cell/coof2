--- a/c:\Users\LENOVO\Desktop\coof2\config-scripts\js_kitchen.js
+++ b/c:\Users\LENOVO\Desktop\coof2\config-scripts\js_kitchen.js
@@ -1,586 +1,529 @@
 /**
  * لوحة العامل (المطبخ): النسخة الكاملة والنهائية
...
-}
-
-// دالة لتهيئة AudioContext عند أول تفاعل للمستخدم
+    tickAllKitchenTimers();
+    if (kitchenTickInterval) clearInterval(kitchenTickInterval);
+    kitchenTickInterval = setInterval(tickAllKitchenTimers, 1000);
+}
...
-        // محاولة استئناف السياق إذا كان معلقاً (ضروري لبعض المتصفحات)
-        if (audioContext.state === 'suspended') {
-            audioContext.resume().then(() => {
-                console.log('AudioContext تم استئنافه بنجاح.');
-            }).catch(e => console.error('خطأ في استئناف AudioContext:', e));
-        }
-    } catch (e) {
-        console.error("خطأ في تهيئة AudioContext:", e);
-        audioContext = null; // إعادة تعيين في حالة الخطأ
-    }
+    } catch (_) {}
 }
...
-    if (!audioContext) {
-        initAudioContext(); // محاولة التهيئة إذا لم تتم بعد
-        if (!audioContext) return;
-    }
-
-    // التأكد من أن السياق يعمل قبل تشغيل الصوت
-    if (audioContext.state === 'suspended') {
-        audioContext.resume().then(() => _playNewOrderSoundLogic()).catch(e => console.error('خطأ في استئناف AudioContext لتشغيل الصوت:', e));
-    } else {
-        _playNewOrderSoundLogic();
-    }
-}
-
-// المنطق الفعلي لتشغيل الصوت
...
+    const payload = { status: "preparing", preparing_started_at: new Date().toISOString() };
+    let result = await updateOrder(orderId, payload, "pending");
+    if (!result.ok) result = await updateOrder(orderId, { status: "preparing" }, "pending");
+
+    if (!result.ok) {
+        setLocalPreparing(order);
+        showRlsBanner();
...
+    if (!client) return alert("النظام غير متصل");
+    let result = await updateOrder(orderId, { status: "ready" }, "preparing");
+    if (!result.ok) result = await updateOrder(orderId, { status: "ready" }, null);
+    if (!result.ok) return alert("تعذر الحفظ: " + result.error);
+
+    clearLocalPreparing(orderId);
+    clearOpenedAt(orderId);
+    localStorage.removeItem(`kitchen_timer_${orderId}`);
...
+    const client = getClient();
+    if (!client) return;
+    client.channel("kitchen_realtime")
...
-}
-
-// إضافة مستمع لحدث تفاعل المستخدم الأول لتهيئة AudioContext
+    testRlsOnStart();
+    loadOrders();
+    setupRealtime();
+}
+
 document.addEventListener('DOMContentLoaded', () => {
     const resumeAudioContext = () => {
         initAudioContext();
-        // إزالة المستمع بعد أول تفاعل لتجنب تكرار التهيئة
